...
6. When an unrecognized username (one not hardcoded as a test username) is provided, an error is displayed but this error display has differences throughout (color, font, sizing, etc) from the original specification. Note the actual vs. scpec in the following images:
...
10. The registration form does not actually work or save any data to a database. Implimentation is not complete.
